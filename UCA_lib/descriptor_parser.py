...
def parse_list_command(arguments):
    ...
    """
    res = OrderedDict([(int(argument.group(1)),argument.group(2)) for argument in LIST_ARGS_PATTERN.finditer(arguments)])
...
def list_command_to_str(arguments):
    ...
    options_str = 'Options: ' + '; '.join(['{0}: {1}'.format(key, option) for key,option in arguments.items()])
...
def checkbox_command_to_str(arguments):
    ...
    unchecked_value = OPPOSITE_STRINGS.get(arguments.lower(), '{0} {1}'.format(DEFAULT_NEGATOR, arguments))
    return "Options: 0 - {0}, 1 - {1}".format(unchecked_value, checked_value)
    
#TODO: Finish this    
def slider_command_to_str(arguments):
    """
    Convert arguments for slider and arrowbox command into comment string for config file.
...
def process_descriptor_for_configfile(descriptor):
    """
    Process descriptors (which may include the descriptor commands) and convert them
    to a readable form.
