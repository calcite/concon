  ...
  if(dev == None):
    ...
    for cfg in dev:
      ...
      for interface in cfg:
        # Test interface class and subclass (generic (0) HID (3) profile)
        if( (interface.bInterfaceClass == 3) and
          ...
          for ep in interface:
            ...
            if( (ep.bLength == 7) and
                ...
                (ep.wMaxPacketSize == 8)):
              # Correct configuration. Test EP direction (if>=128 -> IN)
              if(ep.bEndpointAddress >= 128):
                # EP found -> set flag. Also save endpoint object
                found_in_ep  = 1
                dev_hid.ep_in = ep
              else:
                found_out_ep = 1
                dev_hid.ep_out = ep
          # When here all EP was checked. So check if EP IN and EP OUT are
          # accessible
