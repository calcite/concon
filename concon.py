...
''' % (program_shortdesc, str(__date__))
    ...
    if ((use_first_dev != 0) and (use_first_dev != 1)):
        logger.warning(
            "Parameter use first device have invalid value.\nParameter"
        ...
        if (use_first_dev == 1):
            ...
            while (1):
                for i in range(len(found_devices)):
                    print(" Device number [{0}] Name: {1}".format(i,
                                                                  found_devices[
                                                                      i].name))
                print("<----------------------------------------------->")
                ...
                if (user_choice < 0):
                    print(
                        "Selected option can not be negative. Just type device number.\n"
                        "Try it again ;-)")
                    continue
                if (user_choice > len(found_devices)):
                    print(
                        "How can I select device which is not found? So please type\n"
