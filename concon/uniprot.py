...
class UniprotExceptionResetSuccess(Exception):
    pass


class UniprotExceptionDeviceNotFound(Exception):
    pass


class UniprotExceptionNackFail(Exception):
    pass


class UniprotExceptionRxBufferOverflow(Exception):
    pass
...
class Uniprot(object):
    ...
    def __init__(self, vid, pid, timeout=UsbDriver.USB_TIMEOUT_MS):
        ...
        self._device.open(timeout=timeout)
...
def Uni_process_rx_status_data(buffer_rx):
    global UNI_CHAR_ACK
    global UNI_CHAR_NACK
    global UNI_CHAR_RESET
    global UNI_CHAR_BUFFER_OVERFLOW

    global UNI_RES_CODE_CRC_ERROR
    global UNI_RES_CODE_ACK
    global UNI_RES_CODE_NACK
    global UNI_RES_CODE_DEVICE_BUFFER_OVERFLOW
    global UNI_RES_CODE_RESET
    global UNI_RES_CODE_UNKNOWN_COMMAND
    ...
    if(crc16 != 0):
        return UNI_RES_CODE_CRC_ERROR


    # Test for ACK
    if(buffer_rx[0] == UNI_CHAR_ACK):
        return UNI_RES_CODE_ACK

    # Test for NACK
    if(buffer_rx[0] == UNI_CHAR_NACK):
        return UNI_RES_CODE_NACK

    # Test for RESET
    if(buffer_rx[0] == UNI_CHAR_RESET):
        return UNI_RES_CODE_RESET

    # test for Buffer overflow from device
    if(buffer_rx[0] == UNI_CHAR_BUFFER_OVERFLOW):
        return UNI_RES_CODE_DEVICE_BUFFER_OVERFLOW

    # If no status defined -> FAIL
    return UNI_RES_CODE_UNKNOWN_COMMAND
...
def Uniprot_USB_tx_command( const_UNI_CHAR_CMD ):
    ...
    if(const_UNI_CHAR_CMD == UNI_CHAR_RESET):
        # Add reset symbols (Emergency reset from host)
        i_buffer_tx[3] = UNI_CHAR_RESET
        i_buffer_tx[4] = UNI_CHAR_RESET
        i_buffer_tx[5] = UNI_CHAR_RESET
        i_buffer_tx[6] = UNI_CHAR_RESET
        i_buffer_tx[7] = UNI_CHAR_RESET

    # Check if not ACK -> else probably error -> clear input buffers
    if(const_UNI_CHAR_CMD != UNI_CHAR_ACK):
      # Clear input buffer (just for case)
      Uniprot_USB_clear_rx_buffer(3)

    # Buffer is ready, send data
    UsbDriver.usb_tx_data(device, i_buffer_tx)
...
def Uniprot_USB_try_tx_data( i_tx_data ):
    ...
    global UNI_CHAR_HEADER
    global UNI_CHAR_DATA
    global UNI_CHAR_TAIL
    ...
    while(i_tx_remain_data_Bytes >= 1):
        ...
        while((i_tx_remain_data_Bytes >= 1) and (i_buffer_tx_index <8)):
            ...
            # Test if there is tail
                ...
                i_buffer_tx[i_buffer_tx_index] = UNI_CHAR_TAIL;
                # Calculate CRC
                i_crc16 = \
                    crc_xmodem_update(i_crc16, i_buffer_tx[i_buffer_tx_index])
        ...
        if(i_tx_remain_data_Bytes <= 0):
            ...
            UsbDriver.usb_tx_data(device, i_buffer_tx)
            # Get command
            i_buffer_rx = UsbDriver.usb_rx_data(device)
...
def Uniprot_USB_tx_data( i_tx_data ):
    ...
    global UNI_RES_CODE_RESET
    global UNI_RES_CODE_UNKNOWN_COMMAND
    global UNI_RES_CODE_DEVICE_NOT_FOUND

    global UNI_MAX_NACK_RETRY_COUNT
    ...
    i_nack_cnt = UNI_MAX_NACK_RETRY_COUNT

    # Counter for thrown packets
    i_throw_cnt = 0

    # Send data again if there is NACK or CRC ERROR
    while( (status == UNI_RES_CODE_NACK) or \
           (status == UNI_RES_CODE_CRC_ERROR)
         ):
        ...
        try:
            Uniprot_USB_tx_command(UNI_CHAR_ACK)
        except:
            raise UniprotExceptionDeviceNotFound("[Try TX cmd]"
                                        " Device not found! (loop)\n")

        logger.warn("[Uniprot_USB_tx_data]"
                    " NACK or CRC error. TX data again... (" +
                    str(UNI_MAX_NACK_RETRY_COUNT - i_nack_cnt) +
                    ").")

        try:
            status = Uniprot_USB_try_tx_data( i_tx_data )
        except:
            raise UniprotExceptionDeviceNotFound("[Try TX data]"
                                        " Device not found! (loop)\n")
        ...
        if(i_nack_cnt == 0):
            ...
            raise UniprotExceptionNackFail(" NACK retry count reach limit!\n")


    # Test for other options

    # Test for ACK (standard behaviour)
    if(status == UNI_RES_CODE_ACK):
        return UNI_RES_CODE_SUCCESS


    # Test Exceptions

    # Test for buffer overflow on device -> higher layer should solve this
    if(status == UNI_RES_CODE_DEVICE_BUFFER_OVERFLOW):
        # EXCEPTION
    ...
    if( (status == UNI_RES_CODE_RESET) or \
        (status == UNI_RES_CODE_UNKNOWN_COMMAND)
        ):
        # Restart device
...
def Uniprot_USB_try_rx_data():
    ...
    global UNI_CHAR_DATA
    global UNI_CHAR_TAIL
    global UNI_RES_CODE_ACK_WARNING
    ...
    i_buffer_rx_8 = UsbDriver.usb_rx_data(device)

    logger.debug("[Uniprot_USB_try_rx_data] RAW uniprot data (begin):\n" +
                 str(i_buffer_rx_8) + "\n\n\n")

    # Try to find header
    if((i_buffer_rx_8[0] == UNI_CHAR_HEADER) and
       (i_buffer_rx_8[3] == UNI_CHAR_DATA)):
    ...
    while(i_rx_remain_data_Bytes >= 1):
        ...
        while((i_buffer_rx_8_index < 8) and (i_rx_remain_data_Bytes >= 1)):
            ...
            # If there is Tail
                ...
                # Check Tail itself
                if(i_buffer_rx_8[i_buffer_rx_8_index] == UNI_CHAR_TAIL):
                    # OK, Tail seems to be all right
                    crc16 = crc_xmodem_update(crc16, UNI_CHAR_TAIL)
                else:
                    # Else send NACK
                    logger.debug("[Uniprot_USB_try_rx_data]"
                                 " Tail not found. NACK\n")
    ...
    if(i_warning == 0):
        ...
        return UNI_RES_CODE_ACK_WARNING
...
def Uniprot_USB_rx_data():
    ...
    # Test status
    while((status != UNI_RES_CODE_ACK) and\
          (status != UNI_RES_CODE_ACK_WARNING)):
        ...
        if(status == UNI_RES_CODE_NACK):
            ...
            # Send NACK to device
            try:
                Uniprot_USB_tx_command(UNI_CHAR_NACK)
            except:
        ...
        elif(status == UNI_RES_CODE_RESET):
            # Try send reset and close device
            try:
                Uniprot_USB_tx_command(UNI_CHAR_RESET)
                self.close()
            except:
                # Dummy operation
                pass
    ...
    if(status == UNI_RES_CODE_ACK_WARNING):
...
def Uniprot_USB_clear_rx_buffer(num_of_empty_buffers=2):
  ...
  while(1):
    # Get data
    i_rx_tmp = UsbDriver.usb_rx_data(device)
